...
public:
  ...
  //! Register kind.
  ASMJIT_ENUM(Kind) {
    kKindGp       = Reg::kKindGp,        //!< General purpose register kind.
    kKindVec      = Reg::kKindVec,       //!< Vector (VFP/ASIMD) register kind.
  ...
  ASMJIT_INLINE bool isGp() const noexcept { return _reg.kind == kKindGp; }
  ...
  ASMJIT_INLINE bool isVec() const noexcept { return _reg.kind == kKindVec; }
...
ASMJIT_INLINE uint32_t ArmReg::kindOf(uint32_t regType) noexcept {
  ASMJIT_ASSERT(regType <= Reg::kRegMax);
  return armOpData.archRegs.regInfo[regType].kind;
}
