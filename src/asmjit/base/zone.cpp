...
Error ZoneBits::_resize(size_t newLength, bool newBitsValue) noexcept {
  ...
  if (newLength > _capacity) {
    ...
    BitWord* newData = static_cast<BitWord*>(_heap->alloc(minimumCapacity, allocatedCapacity));
    ...
    if (data)
      _heap->release(data, _capacity / 8);
    data = newData;
...
UNIT(base_zonevector) {
  ...
  ZoneVector<int> vec(&heap);

  INFO("ZoneVector<int> basic tests");
  EXPECT(vec.append(0) == kErrorOk);
...
UNIT(base_zonebits) {
  ...
  for (count = 1; count < kMaxCount; count++) {
    ...
    vec.clear();
    EXPECT(vec.resize(count, true) == kErrorOk);
    EXPECT(vec.getLength() == count);
