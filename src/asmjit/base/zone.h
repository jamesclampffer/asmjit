...
public:
  ...
  ASMJIT_INLINE Error willGrow(size_t n = 1) noexcept {
...
public:
  ...
  ASMJIT_INLINE size_t getCapacity() const noexcept { return _length; }
  ...
  ASMJIT_API Error _resize(size_t newLength, bool newBitsValue = false) noexcept;
  ASMJIT_API Error _fill(size_t fromIndex, size_t toIndex, bool value) noexcept;
  ...
  ASMJIT_INLINE Error resize(size_t newLength, bool newBitsValue = false) noexcept {
    ASMJIT_ASSERT(isInitialized());
    return _resize(newLength, newBitsValue);
  }

  ASMJIT_INLINE Error truncate(size_t newLength) noexcept {
    ASMJIT_ASSERT(isInitialized());
    ASMJIT_ASSERT(newLength <= _length);

    return _resize(newLength);
  }
